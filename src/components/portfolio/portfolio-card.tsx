  ...
  return (
    <Card className="flex flex-col h-full overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 ease-out hover:-translate-y-1 hover:scale-[1.02] rounded-lg">
      ...
      <CardContent className="p-6 flex-grow">
        <CardTitle className="font-headline text-2xl text-primary mb-2">{project.title || 'Untitled Project'}</CardTitle>
        <div className="mb-4 space-x-2 flex flex-wrap gap-y-2">
          {projectTags.map((tag) => (
            <Badge key={tag} variant="secondary" className="font-normal">{tag}</Badge>
          ))}
        </div>
        <CardDescription className="text-muted-foreground line-clamp-3">{project.description || 'No description available.'}</CardDescription>
      </CardContent>
