...
const defaultFormValues: SkillAdminFormData = {
  id: undefined,
  name: commonSkillNames.length > 0 ? commonSkillNames[0] : '',
...
function SkillsAdminClientPage({ initialSkills }: { initialSkills: Skill[] }) {
  ...
  useEffect(() => {
    if (formActionState.status === 'success' && formActionState.savedSkill) {
      ...
      setSkills(prevSkills => {
        ...
        let newSkillsArray = existingIndex > -1
          ? prevSkills.map(s => (s.id === savedSkill.id ? savedSkill : s))
          : [savedSkill, ...prevSkills];
        return newSkillsArray.sort((a, b) => a.category.localeCompare(b.category) || a.name.localeCompare(b.name));
      });
      setShowForm(false);
      ...
      if (formActionState.errors) {
        Object.entries(formActionState.errors).forEach(([key, fieldErrorMessages]) => {
          if (Array.isArray(fieldErrorMessages) && fieldErrorMessages.length > 0) {
            form.setError(key as Path<SkillAdminFormData>, { type: 'server', message: fieldErrorMessages.join(', ') });
          }
        });
      }
    }
  ...
  return (
    <div className="py-6">
      <div className="flex items-center justify-between">
        ...
        {!showForm && (
          <Button onClick={handleAddNew}><PlusCircle className="mr-2 h-4 w-4" /> Add New Skill</Button>
        )}
      </div>

      {showForm && (
        <Card key={formCardKey}> 
          <CardHeader><CardTitle>{currentSkill ? 'Edit Skill' : 'Add New Skill'}</CardTitle><CardDescription>Select a skill and its details.</CardDescription></CardHeader>
          <Form {...form}>
            <form action={dispatchServerAction}>
              ...
              <CardContent className="space-y-6">
                <FormField control={form.control} name="name" render={({ field }) => (
                  <FormItem>
                    ...
                    <Select onValueChange={field.onChange} value={field.value} name={field.name}>
                      ...
                      <SelectContent>{commonSkillNames.map(skillName => (<SelectItem key={skillName} value={skillName}>{skillName}</SelectItem>))}</SelectContent>
                    </Select>
                    <FormMessage />
                  </FormItem>
                )} />
                ...
                <FormField control={form.control} name="proficiency" render={({ field: { onChange, onBlur, value, name, ref } }) => (
                  <FormItem>
                    <FormLabel>Proficiency (0-100, optional)</FormLabel>
                    <FormControl><Input type="number" name={name} value={value ?? ''} onChange={e => onChange(e.target.value === '' ? undefined : Number(e.target.value))} onBlur={onBlur} ref={ref} min="0" max="100" /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
              </CardContent>
      ...
      {!showForm && (
        <div className="space-y-8">
          {skills.length === 0 && <p className="text-muted-foreground text-center py-4">No skills yet. Click "Add New Skill" to start.</p>}
          {skillCategories.map(category => {
            const categorySkills = skills.filter(skill => skill.category === category);
            if (categorySkills.length === 0) return null;
            return (
              <div key={category}>
                <h2 className="text-2xl font-semibold mb-4 text-primary border-b pb-2">{category}</h2>
                    ...
                    return (
                      <Card key={skill.id} className="flex items-center justify-between p-4 hover:shadow-md transition-shadow">
                        <div className="flex items-center gap-3">
                          ...
                          {skill.proficiency != null && <span className="text-sm text-muted-foreground">({skill.proficiency}%)</span>}
                        </div>
                        <div className="space-x-2">
                          <Button variant="outline" size="sm" onClick={() => handleEdit(skill)}><Edit3 className="mr-1 h-4 w-4" /> Edit</Button>
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader><AlertDialogTitle>Are you sure?</AlertDialogTitle><AlertDialogDescription>This action cannot be undone. This will permanently delete the skill "{skill.name}".</AlertDialogDescription></AlertDialogHeader>
                              <AlertDialogFooter><AlertDialogCancel>Cancel</AlertDialogCancel><AlertDialogAction onClick={() => handleDelete(skill.id)} className="bg-destructive hover:bg-destructive/80">Yes, delete skill</AlertDialogAction></AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        </div>
                      </Card>
